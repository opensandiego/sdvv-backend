...
def load_sheets_from_years(years, sheet_names):
    ...
    for year in years:
        for sheet in sheet_names:
            sheets.append(
                pandas.read_excel("static/efile_SD_CSD_" + str(year) + ".xlsx", sheet)
            )
    return sheets
...
def export_contribution_info_to_json():
    ...
    expenditures_by_zip = unique_sheet_columns_sum(
        "Tran_Zip4", "Tran_Amt2", *contribution_sheets
    )
    expenditures_by_occupation = unique_sheet_columns_sum(
...
if __name__ == "__main__":
    export_contribution_info_to_json()
